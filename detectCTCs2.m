...
X = bwlabel(I2w); % connect component - each has a number
stats = regionprops(X,'all');
% TEMPLATE MATCHING FOR DAPI
...
goodFeats = find(OUT(:,1)<(V*0.1)); % SPOTS WHICH FIT WELL WITH THE MEAN CTC SPOT
...
for i = 1:length(feats)
    plot(feats(i).pos(1),feats(i).pos(2),'rs')
    text(feats(i).pos(1)+2,feats(i).pos(2)+2,[num2str(i)],'Color','r');
end
plot(x,y,'g*')
% PERFORM UNIMODAL THRESHOLING FOR DAPI PIXELS AFTER WAVELET SEGMENTATION
...
% DO TEMPLATE MATHCING OF ALL CK BLOBS AFTER DAPI SELECTION 

% TWO LOOPS - 1. for those that have centroid CDw45 = 0
% 2. for those that have some signal - RATIO decision
